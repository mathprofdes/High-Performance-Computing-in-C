...
int main(int argc, char *argv[]) {
	int commsz, rank, n;
	MPI_Status status;
	double piapprox = 0, local_piapprox = 0, spiapprox = 0;
	...
	if (rank == 0) {
		...
		starttime = MPI_Wtime();
		spiapprox = localApprox(n, 0, 1);
		endtime = MPI_Wtime();

		printf("Serial pi is approximately %.16f, Error is %.16f\n", spiapprox,
				fabs(spiapprox - PI25DT));
		printf("Time %.6f sec.\n", endtime - starttime);
	}

	// Block all processes until process 0 is finished with the serial computation.
