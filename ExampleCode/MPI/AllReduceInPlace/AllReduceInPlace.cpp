...
// Send size of array to all processes.
	...
	// This version will do the reduction and then update mu with the
	// result on all processors.  Here there is not a need for a local mu.
	MPI_Allreduce(MPI_IN_PLACE, &mu, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
