...
int main(int argc, char *argv[]) {
	...
	if (rank < commsz - 1) {
		localA = new double[sendsize];
		lyp = new double[bufentries];
		MPI_Scatterv(A, counts, disp, MPI_DOUBLE, localA, sendsize, MPI_DOUBLE,
				0, MPI_COMM_WORLD);
	} else {
		localA = new double[lastsendsize];
		lyp = new double[bufentries];
		MPI_Scatterv(A, counts, disp, MPI_DOUBLE, localA, lastsendsize,
	...
	for (int k = 0; k < iters; k++) {
		...
		// Note that the last process has a different size then the rest.
		if (rank < commsz - 1) {
			MPI_Gatherv(lyp, sendsize / cols, MPI_DOUBLE, yp, vcounts, vdisp,
			MPI_DOUBLE, 0, MPI_COMM_WORLD);
		} else {
			MPI_Gatherv(lyp, lastsendsize / cols, MPI_DOUBLE, yp, vcounts,
					vdisp, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		}
