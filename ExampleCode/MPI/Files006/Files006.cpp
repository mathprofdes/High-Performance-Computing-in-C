...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < 5; i++)
		fileportion[i] = rand() % 1000;

	MPI_File_seek(file, rank * 5 * sizeof(int), MPI_SEEK_SET);
	MPI_File_write_all(file, fileportion, 5, MPI_INT, MPI_STATUS_IGNORE);

	cout << "Portion Rank " << rank << ": ";
	for (int i = 0; i < 5; i++) {
		cout << fileportion[i] << " ";
	}
	cout << endl;
...
// Make sure that all writes are done before reading.
	...
	MPI_File_read_all(file, filecontents, 5 * commsz, MPI_INT,
	MPI_STATUS_IGNORE);
